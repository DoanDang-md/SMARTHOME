...
# Route cố định phải khai báo TRƯỚC /{device_id} kẻo "latest" bị nuốt
@router.get("/latest/{device_id}")
def get_latest_ir_learned(device_id: int, db: Session = Depends(get_db)):
    return IRService(db).get_latest(device_id)


@router.post("/sync/{device_id}", response_model=list[schemas.IRCommandResponse])
def force_sync_ir(
    device_id: int,
    db: Session = Depends(get_db),
    current_user: models.User = Depends(get_current_user),
):
    """Ép đồng bộ 2 chiều Gateway ↔ Backend (admin/user đã login)."""
    return IRService(db, current_user).sync_and_get_ir(device_id)
