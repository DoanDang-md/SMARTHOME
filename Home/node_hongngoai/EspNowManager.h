 ...
 * @brief ESP-NOW manager — Node IR (ESP32-C3 SuperMini).
 *
 * Học từ Hybrid (ESP8266→ESP32 Gateway, log đã chứng minh):
 * - Unicast Node→GW có thể không tới; Broadcast luôn ổn định hơn.
 * - Unicast GW→Node (lệnh LEARN/BLAST/SAVE + ACK) vẫn OK.
...
public:
...
#if defined(ESP32)
        // Tắt modem sleep — tránh rơi RX lệnh LEARN/BLAST từ Gateway
        WiFi.setSleep(false);
...
#endif
        Serial.println("[ESPNOW] Uplink = BROADCAST (tránh unicast Node→GW fail như Hybrid)");
        return true;
    }

    /**
     * Gửi lên Gateway qua BROADCAST.
     * gatewayMac_ unicast chỉ dùng nhận diện / log — không làm dest TX.
     */
...
#endif
        if (!ok) {
            Serial.printf("[ESPNOW TX] LỖI cmd=0x%02X ir=0x%08X\n",
                          command, static_cast<unsigned>(irData));
            return false;
        }
        if (command == smarthome::CMD_IR_DATA) {
            Serial.printf("[ESPNOW TX] IR EVENT 0x%08X dest=BCAST ch=%u locked=%d\n",
                          static_cast<unsigned>(irData), config_.channel,
                          gatewayLocked_ ? 1 : 0);
        } else if (command == smarthome::CMD_ACK_REPORT
                   || command == smarthome::CMD_DISCOVERY) {
            Serial.printf("[ESPNOW TX] cmd=0x%02X type=%u ir=0x%08X dest=BCAST "
                          "ch_cfg=%u ch_hw=%u locked=%d gw=%02X:%02X:%02X:%02X:%02X:%02X\n",
                          command, deviceType, static_cast<unsigned>(irData),
            ...
            Serial.printf("[ESPNOW TX] cmd=0x%02X ir=0x%08X dest=BCAST ok\n",
                          command, static_cast<unsigned>(irData));
        }
        return true;
    }
    ...
    void softRecoverLink(DiscoverySendFn sendDiscovery, void* ctx) {
        Serial.printf("\n[WIFI] Soft recover IR (ch=%u) — Discovery BCAST\n", config_.channel);
        prepareRadioForTx();
        if (sendDiscovery != nullptr) {
            sendDiscovery(ctx);
        }
    ...
    /**
     * LEARN_IR: ghi nhớ MAC nguồn lệnh (GW).
     * Không xóa peer broadcast — uplink vẫn cần BCAST.
     */
    void ensureGatewayMac(const uint8_t* mac) {
        if (mac == nullptr) return;
        if (memcmp(gatewayMac_, mac, 6) == 0) {
            setWifiChannel(config_.channel);
            ensureBroadcastPeer();
            return;
        }
        memcpy(gatewayMac_, mac, 6);
        gatewayLocked_ = true;
        lastGatewayRxMs_ = millis();
        setWifiChannel(config_.channel);
        ensureBroadcastPeer();
        Serial.printf("[ESPNOW] ensureGatewayMac: %02X:%02X:%02X:%02X:%02X:%02X (TX vẫn BCAST)\n",
...
private:
    ...
    /** Chỉ coi gói GW→Node (tránh khóa nhầm kênh / nhiễu). */
    ...
    /**
     * Ghi nhớ MAC Gateway từ ACK/lệnh. KHÔNG xóa peer broadcast.
     */
    ...
    uint8_t gatewayMac_[6];   // MAC GW từ RX — không dùng dest TX
