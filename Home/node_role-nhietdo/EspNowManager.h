 ...
 *
 * QUAN TRỌNG (ESP8266 Hybrid → ESP32 Gateway, đã chứng minh bằng log):
 * - Unicast Node→GW (tới MAC STA Gateway) thường KHÔNG được RX.
 * - Broadcast FF:FF:FF:FF:FF:FF luôn tới được.
 * - Unicast GW→Node (ACK/lệnh) vẫn OK.
 * → Uplink luôn gửi BROADCAST; gatewayMac_ chỉ để nhận diện/log khi lock.
 * → Score-scan kênh (hits) — không first-hit (tránh bleed kênh kề).
 */
#pragma once
